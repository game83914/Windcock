<template>
  <div>
    ...
    <div class="grid grid-cols-3 border-2 border-[#171717]" role="tablist" aria-label="我的議題分類">
      <button
        ...
        type="button"
        role="tab"
        class="focus-ring min-h-12 px-2 text-xs font-black sm:px-4 sm:text-sm"
        :class="tab === option.value ? 'bg-[#171717] text-white' : 'bg-[#faf8f3] text-[#5f5a53]'"
    ...
    <div v-if="tab === 'topics'" id="topics-panel" role="tabpanel" aria-labelledby="topics-tab">
      <div v-if="topicsLoading" class="mt-5 space-y-3">
        <div v-for="item in 3" :key="item" class="h-36 animate-pulse bg-[#e5e0d6]" />
      </div>
      <p v-else-if="topicsError" class="mt-5 border-l-4 border-[#d84a36] bg-[#fbe9e5] p-4 text-sm">{{ topicsError }}</p>
      <div v-else-if="topics.length" class="mt-5 space-y-4">
      ...
      <div v-else class="mt-5 border border-[#d7d1c6] bg-[#faf8f3] px-6 py-16 text-center">
        <h2 class="text-xl font-black">你還沒有發起議題</h2>
        <p class="mt-2 text-sm text-[#77716a]">你送出的議題提案請見 <button type="button" class="focus-ring font-black text-[#3157d5]" @click="setTab('proposals')">議題提案</button> 分頁。</p>
      </div>
    </div>

    <div v-else-if="tab === 'proposals'" id="proposals-panel" role="tabpanel" aria-labelledby="proposals-tab">
    ...
    <div v-else id="stances-panel" role="tabpanel" aria-labelledby="stances-tab">
...
import type { Category, Topic, TopicListResponse } from '~/types/topic';
import { applyCategoryRules, deadlineLabel, formatCompactNumber, getCategoryMeta } from '~/utils/topic';

definePageMeta({ middleware: 'auth' });
useSeoMeta({ title: '我的議題｜輿論測風向' });

interface TopicApplication { id: string; title: string; description: string | null; category: string; topicType: 'BINARY' | 'MULTIPLE' | 'SPECTRUM'; audience: 'MEMBER_ONLY' | 'FOLLOWERS_ONLY'; options: string[] | null; blocks: unknown[] | null; voteDurationDays: number; status: string; note: string | null; createdAt: string; updatedAt: string; results: Array<{ topicId: string; topic: { id: string; title: string } }>; organization: { id: string; name: string } | null }
interface TopicRevision { id: string; revisionNumber: number; status: string; reviewNote: string | null }

type ProposalTab = 'topics' | 'proposals' | 'stances';
...
const tabOptions: Array<{ value: ProposalTab; label: string }> = [
  { value: 'topics', label: '已建立議題' },
  { value: 'proposals', label: '議題提案' },
  { value: 'stances', label: '立場提案' },
];

function tabFromQuery(value: unknown): ProposalTab | null {
  return typeof value === 'string' && tabOptions.some((option) => option.value === value) ? (value as ProposalTab) : null;
}

const userPickedTab = ref(false);
const tab = ref<ProposalTab>(tabFromQuery(route.query.tab) ?? 'topics');

async function setTab(next: ProposalTab) {
  userPickedTab.value = true;
  if (next === tab.value) return;
  await router.replace({ query: { tab: next } });
}
...
watch(() => auth.capabilitySummary, (summary) => {
  if (!summary || userPickedTab.value || tabFromQuery(route.query.tab) || auth.canAuthorTopics) return;
  tab.value = 'proposals';
  void loadActive();
});
...
async function loadActive() {
  if (tab.value === 'topics') await loadMyTopics();
  else if (tab.value === 'proposals') await loadProposals();
  else await loadStances();
}

async function loadMyTopics(force = false) {
  if (topicsLoaded.value && !force) return;
  topicsLoading.value = true; topicsError.value = '';
  try { topics.value = (await api.get<TopicListResponse>('/me/topics', { limit: 50 })).items; topicsLoaded.value = true; }
  catch (cause) { topicsError.value = errorMessage(cause); }
...
async function saveTopic(item: TopicApplication) {
  topicError.value = '';
  try {
    await api.patch(`/topic-applications/${item.id}`, { ...topicDraft, description: topicDraft.description || undefined, note: topicDraft.note || undefined, topicType: item.topicType, audience: item.audience, options: item.options || undefined, blocks: item.blocks || undefined, voteDurationDays: item.voteDurationDays, expectedUpdatedAt: item.updatedAt });
    topicEditingId.value = ''; await loadProposals(true);
  } catch (cause) { topicError.value = errorMessage(cause); }
}
