<template>
  <div>
    <div class="mb-8 border-b-2 border-[#171717] pb-6">
      <h1 class="mt-2 text-3xl font-black tracking-[-0.04em]">我的問卷</h1>
    </div>
    ...
    <div v-else-if="topics.length" class="space-y-4">
      <article v-for="topic in topics" :key="topic.id" class="border border-[#d7d1c6] bg-[#faf8f3] p-5 sm:p-6">
        <div class="flex flex-wrap items-center gap-2 text-xs">
          <span class="bg-[#b0761f] px-2 py-1 font-bold text-white">問卷</span>
          <span
        ...
        <div class="mt-3 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <p class="mt-2 text-xs text-[#77716a]">{{ topic.surveyQuestionCount ?? 0 }} 題 · {{ topic.voteEndAt ? deadlineLabel(topic.voteEndAt) : '尚未開票' }}</p>
          </div>
    ...
    <div v-else class="border border-[#d7d1c6] bg-[#faf8f3] px-6 py-16 text-center">
      <h2 class="text-xl font-black">你還沒有發起問卷</h2>
    </div>
...
import { deadlineLabel } from '~/utils/topic';

definePageMeta({ middleware: 'auth' });
useSeoMeta({ title: '我的問卷｜輿論測風向' });
...
onMounted(async () => {
  try {
    const res = await api.get<{ items: Topic[] }>('/topics/me/surveys', { limit: 50 });
