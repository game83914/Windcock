<template>
  <div>
    <div class="mb-8 flex flex-col gap-4 border-b-2 border-[#171717] pb-6 sm:flex-row sm:items-end sm:justify-between">
      ...
      <NuxtLink to="/topics/quick" class="focus-ring bg-[#b0761f] px-5 py-3 text-center text-sm font-bold text-white">＋ 發起快問</NuxtLink>
    </div>
    ...
    <div v-else-if="topics.length" class="space-y-4">
      <article v-for="topic in topics" :key="topic.id" class="border border-[#d7d1c6] bg-[#faf8f3] p-5 sm:p-6">
        ...
        <div class="mt-3 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          ...
          <NuxtLink :to="`/topic/${topic.id}`" class="focus-ring shrink-0 text-sm font-bold text-[#b0761f]">查看詳情 &rarr;</NuxtLink>
        </div>
    ...
    <div v-else class="border border-[#d7d1c6] bg-[#faf8f3] px-6 py-16 text-center">
      ...
      <NuxtLink to="/topics/quick" class="focus-ring mt-5 inline-block bg-[#b0761f] px-6 py-3 text-sm font-bold text-white">發起第一則快問</NuxtLink>
    </div>
  </div>
</template>
